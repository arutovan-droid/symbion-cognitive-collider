...
async def test_epistemological_nuance():
    ...
    for q in queries:
        ...
        # Показываем топ-3 с индексами (r.top_candidates — список словарей)
        top3 = []
        for i, item in enumerate(r.top_candidates[:3]):
        ...
        if 'arc_rank' in q['expect']:
            arc_pos = next((i for i, item in enumerate(r.top_candidates[:3]) if item.get('lang') == 'arc'), None)
        ...
        if 'hy_rank' in q['expect']:
            hy_pos = next((i for i, item in enumerate(r.top_candidates[:3]) if item.get('lang') == 'hy'), None)
